...
void startCameraServer() {
  ...
  httpd_uri_t green_uri = {
    ...
    .handler = [](httpd_req_t *req){
      ...
      httpd_resp_send(req, "GREEN", HTTPD_RESP_USE_STRLEN);
      return ESP_OK;
    }
  };
  ...
  httpd_uri_t red_uri = {
    ...
    .handler = [](httpd_req_t *req){
      ...
      httpd_resp_send(req, "RED", HTTPD_RESP_USE_STRLEN);
      return ESP_OK;
    }
  };

  // SPOOF ALERT
  httpd_uri_t spoof_uri = {
    .uri = "/spoof",
    .method = HTTP_GET,
    .handler = [](httpd_req_t *req){
      digitalWrite(2, LOW);
      ...
      httpd_resp_send(req, "SPOOF", HTTPD_RESP_USE_STRLEN);
      return ESP_OK;
    }
  };

  httpd_register_uri_handler(server, &stream_uri);
  httpd_register_uri_handler(server, &green_uri);
  httpd_register_uri_handler(server, &red_uri);
  httpd_register_uri_handler(server, &spoof_uri);
}
